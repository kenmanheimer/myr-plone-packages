...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t kenmanheimer/myr-plone-packages-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t kenmanheimer/myr-plone-packages-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name myr-plone-packages-backend-acceptance -d kenmanheimer/myr-plone-packages-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name myr-plone-packages-frontend-acceptance --link myr-plone-packages-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d kenmanheimer/myr-plone-packages-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop myr-plone-packages-frontend-acceptance
	@docker stop myr-plone-packages-backend-acceptance
